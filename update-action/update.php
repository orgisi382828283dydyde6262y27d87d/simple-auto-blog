...
/* Declaring variables */
$root = getcwd();
$rss_feed = 'https://api.rss2json.com/v1/api.json?rss_url='.urlencode('https://habr.com/en/rss/all/all/');
print_r($rss_feed);
...
foreach ($rss_feed_f->items as $post){
    $title = $post->title;
    $furl = str_replace(':','',str_replace(';','',str_replace('&','',str_replace('[','',str_replace(']','',str_replace(' ','-',str_replace('.','-',($title))))))));
