...
foreach ($rss_feed_f->items as $post){
    ...
    echo file_get_contents('http://api.carente.eu.org/?key='.$queue_url.'&request='.urlencode(base64_encode('https://script.google.com/macros/s/AKfycby6XK_viaR-FmH-s6-IRh-V3-_yFCcigSDxF86LTDZ3XaN4kC5ovOymg8HZEmf2i4x1/exec?key='.urlencode($api_key2).'&path='.urlencode($furl).'&url='.urlencode($links))));
